        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6 mb-6 flex gap-4 items-end">
          <div>
            ...
            <input required value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })}
              className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="FRY-XXX" />
          </div>
            ...
            {products.map((p) => {
              ...
              return (
                <tr key={p.id} className="border-b last:border-0 hover:bg-zinc-50">
                  ...
                  <td className="p-4 text-orange-600">{p.reserved_quantity > 0 ? p.reserved_quantity : "—"}</td>
                  <td className="p-4">
                    <span className={`font-bold ${available < 50 ? "text-red-600" : "text-green-600"}`}>
                      {available}
                    </span>
                  </td>
                </tr>
